...
router.get('/admin/users/edit/:userId', isAdmin, async (req, res) => {
  ...
  const userIpData = ipRegistry.find(entry => entry.userId === userId) || { ip: 'Unknown' };

  // Fetch fingerprint data
  const fingerprintRegistry = await db.get('fingerprint_registry') || [];
  const userFingerprintData = fingerprintRegistry.find(entry => entry.userId === userId);
  
  let fingerprint = 'Unknown';
  let fingerprintDetails = null;
  
  if (userFingerprintData) {
    fingerprint = userFingerprintData.fingerprint;
    try {
      fingerprintDetails = JSON.parse(fingerprint);
    } catch (e) {
      console.error('Error parsing fingerprint data:', e);
    }
  }
...
router.get('/admin/users', isAdmin, async (req, res) => {
  try {
    ...
    // Get IP registry data
    const ipRegistry = await db.get('ip_registry') || [];
    
    // Enhance user objects with IP data
    users = users.map(user => {
      const ipData = ipRegistry.find(entry => entry.userId === user.userId);
      return {
        ...user,
        ip: ipData ? ipData.ip : 'Unknown',
        registrationDate: ipData ? ipData.registrationDate : null
      };
    });
