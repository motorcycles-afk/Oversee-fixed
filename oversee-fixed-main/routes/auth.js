...
async function generateFingerprint(req) {
  ...
  // Create a fingerprint based on user agent data
  const fingerprint = {
...
async function createUser(username, email, password, req) {
  ...
  return addUserToUsersTable(username, email, password, true, req);
}

async function addUserToUsersTable(username, email, password, verified, req) {
  try {
    const hashedPassword = await bcrypt.hash(password, saltRounds);
    const userId = uuidv4();
    let users = await db.get('users') || [];
    const newUser = { userId, username, email, password: hashedPassword, accessTo: [], admin: false, welcomeEmailSent: false, verified };
    users.push(newUser);
    ...
    await db.set('ip_registry', ipRegistry);

    // Store browser fingerprint
    const fingerprint = await generateFingerprint(req);
    let fingerprintRegistry = await db.get('fingerprint_registry') || [];
    fingerprintRegistry.push({
    ...
    await db.set('fingerprint_registry', fingerprintRegistry);
